...
VarianceFlowNode::VarianceFlowNode( const string& Name ) : MonteCarloNode( Name )
{
}

VarianceFlowNode::~VarianceFlowNode() = default;
...
void VarianceFlowNode::ComputeValue( size_t DateIndex )
{
    ...
    //! Variance::ObservationDriftVariance.
...
void VarianceFlowNode::SetPastVariance( double PastSum2 )
{
    _past_variance = PastSum2;
}

void VarianceFlowNode::SetTotalYearFraction( double TotalYearFraction )
{
...
void VarianceFlowNode::GetDateDependencies( size_t DateIndex,
...
void VarianceFlowNode::SetSpotNode( MonteCarloNode* N )
{
    _spot_node = N;
}

//! set the strike variance K_var (decimal^2).
void VarianceFlowNode::SetStrikeVariance( double StrikeVariance )
{
    _strike_variance = StrikeVariance;
}

//! set the variance notional (cash per unit of variance).
void VarianceFlowNode::SetNotional( double Notional )
{
    _notional = Notional;
}

//! set the maturity (flow) date index.
void VarianceFlowNode::SetFlowDateIndex( size_t DateIndex )
{
    _flow_date_index = DateIndex;
}

//! set the discrete fixing schedule (sorted date indices; empty = continuous).
void VarianceFlowNode::SetObservationDateIndices( const vector<size_t>& DateIndices )
{
