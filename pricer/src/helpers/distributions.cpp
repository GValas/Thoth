...
{
    //! Phi(x) = erfc(-x/sqrt(2))/2 — exact (M_SQRT1_2 = 1/sqrt(2)); erfc keeps the
    //! left tail accurate (no catastrophic cancellation as x -> -inf)
...
double NormalCdfInv( double P )
{
    ...
    const double p_low = 0.02425;      //!< lower break between tail and central region
    const double p_high = 1.0 - p_low; //!< upper break (symmetric)

    //! endpoints map to the infinite quantiles
    if ( P <= 0.0 )
    {
        return -HUGE_VAL;
    }
    ...
    //! lower tail: change of variable q = sqrt(-2 ln P), rational c/d approximation
    if ( P < p_low )
    {
    ...
    //! central region: rational a/b approximation in r = (P-0.5)^2
    if ( P <= p_high )
    {
        double q = P - 0.5;
        double r = q * q;
    ...
    //! upper tail: mirror of the lower-tail branch on 1-P, result negated
