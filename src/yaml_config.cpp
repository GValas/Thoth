...
gsl_vector* YamlConfig::GetGslVector( const string& Path )
{
    YAML::Node s;
    try
    {
    ...
    GslVector v = gsl_vector_alloc( s.size() );
    try
    {
        for ( size_t i = 0; i < s.size(); i++ )
        {
            gsl_vector_set( v, i, s[i].as<double>() );
        }
    }
    catch ( ... )
    {
...
{
    SetDoubleList( Path, Value->block->data, Value->block->size );
}
